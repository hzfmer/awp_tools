...
int main (int argc, char *argv[]) {
   ...
   int itype=0; 
   int k, l, n;
   float *PH, *PZ, *PX, vh, vx, vz, *RT;
   float *bufx, *bufz, **x, **z;
   char pxfile[100], pzfile[100], pfile[100], rtfile[100];
   long int s0;
   int xi, yi, zi;
   MPI_Offset off;
   MPI_File pxfid, pzfid, pfid, rtfid;
   ...
   /* threshold on slip rate to define rupture time */
   float thres = 1.e-3;
   ...
   while ((c = getopt (argc, argv, "fl:d:t:")) != -1) {
     switch(c) {
        case 'f':
           dofilt = 1;
           break;
        ...
        case 'd':
           ...
           break;
        case 't':
           sscanf(optarg, "%f", &thres);
           break;
        case '?':
           fprintf(stdout, "usage: %s [-f] [-l lp] [-d dt] [-t th]\n", argv[0]);
           exit(0);
        default:
           abort();
     }
  ...
  if (rank==0) fprintf(stdout, "dofilt=%d, lp=%f\n", dofilt, lp);
   ...
   RT=(float*) calloc(csize, sizeof(float));
   for (l=0; l<csize; l++) RT[l] = 1.e9;
   ...
   sprintf(rtfile, "output_dyn/ruptime.bin");
   ...
   MPI_File_open(MPI_COMM_WORLD, pfile, MPI_MODE_CREATE | MPI_MODE_WRONLY, 
       MPI_INFO_NULL, &pfid);
   MPI_File_open(MPI_COMM_WORLD, rtfile, MPI_MODE_CREATE | MPI_MODE_WRONLY, 
       MPI_INFO_NULL, &rtfid);
   ...
   for (k=0; k<nchunks; k++){
      ...
      for (l=0; l<csize; l++){
         ...
         for (n=0; n<nt; n++){
             ...
             vz=fabsf(z[l][n]);
             if (PZ[l] < vz) PZ[l] = vz;
             if ((vh > thres) && (RT[l] == 1.e9)) RT[l] = n*dt2;
         }
      }
      ...
      MPI_File_write_at_all(rtfid, off, RT, csize, MPI_FLOAT, MPI_STATUS_IGNORE);
   }

   MPI_File_close(&pxfid); 
   MPI_File_close(&pzfid); 
   MPI_File_close(&pfid); 
   MPI_File_close(&rtfid);
