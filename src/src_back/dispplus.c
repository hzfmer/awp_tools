...
int main (int argc, char *argv[] ){
  ...
  nz=z1-z0+1;
   ...
   sprintf(xfile, "output_dyn/uplus");
   sprintf(zfile, "output_dyn/wplus");
   ...
   if (dofilt == 0){
      sprintf(dxfile, "disp_plus_X.bin");
      sprintf(dzfile, "disp_plus_Z.bin");
   }
   else {
      sprintf(dxfile, "disp_plus_X_%04.1fHz.bin", lp);
      sprintf(dzfile, "disp_plus_Z_%04.1fHz.bin", lp);
   }
