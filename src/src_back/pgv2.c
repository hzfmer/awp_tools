...
int main (int argc, char *argv[] ){
  ...
  if (rank==0) fprintf(stdout, "dofilt=%d, lp=%f\n", dofilt, lp);
   ...
   err=MPI_File_open(MPI_COMM_WORLD, dxfile, MPI_MODE_CREATE | MPI_MODE_WRONLY, 
       MPI_INFO_NULL, &dxfid);
   error_check(err, "MPI_File_open dxfile");

   err=MPI_File_open(MPI_COMM_WORLD, dyfile, MPI_MODE_CREATE | MPI_MODE_WRONLY, 
       MPI_INFO_NULL, &dyfid);
   error_check(err, "MPI_File_open dyfile");

   err=MPI_File_open(MPI_COMM_WORLD, dzfile, MPI_MODE_CREATE | MPI_MODE_WRONLY, 
       MPI_INFO_NULL, &dzfid);
