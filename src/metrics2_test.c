...
MPI_Datatype data_type(const struct Mpi *mpi, int nx, int ny, int nt, int ysplit, int yoff);
...
int main(int argc, char *argv[])
{
    ...
    for (i = 1; i <= ysplit; ++i) {
        MPI_Datatype read_dtype = data_type(&m, nx, ny, size[2], ysplit, (i - 1) * (ny / ysplit));

        if (m.rank == 0) {
            fprintf(stdout, "rank=%ld: (nxt, nyt)=(%d, %d), yoff=%d, fname=%s, csize=%d, buffer_size=%ld\n", m.rank, m.nxt, m.nyt, i * ny / ysplit, fname, csize, buffer_size);
            fflush(stdout);
        }
...
MPI_Datatype data_type(const struct Mpi *mpi, int nx, int ny, int nt, int ysplit, int yoff)
{
    ...
    old[2] = nx;
    new[0] = nt;
    new[1] = mpi->nyt / ysplit;
    new[2] = mpi->nxt;
    offset[0] = 0;
    offset[1] = new[1] * mpi->coord[1] + yoff;
    offset[2] = new[2] * mpi->coord[0];

    MPI_Datatype readtype;
    fprintf(stdout, "rank=%d, coord=(%d, %d), old=(%d %d %d), new=(%d %d %d), offset=(%d %d %d)\n", mpi->rank, mpi->coord[0], mpi->coord[1], old[0], old[1], old[2], new[0], new[1], new[2], offset[0], offset[1], offset[2]);
    fflush(stdout);
