...
int main(int argc, char **argv)
{
        ...
        if (m.rank == 0) {
                ...
                printf(
                    ...
                    py, mesh_out, rpt);
                long int file_size = (long int) nvars * nx * ny * nz * sizeof(prec);
        ...
        if (m.rank == 0) {
            fprintf(stdout, "Opened output file. In each GPU, nxt=%d, nyt=%d, nzt=%d, Buffersize=%d\n", m.nxt, m.nyt, m.nzt, buffer_size);
            fflush(stdout);
        }
        float *buffer = (float*) calloc(buffer_size, sizeof(float));  // coordinates
        float *buffer_m = (float*) calloc(buffer_size, sizeof(float));  // material
        float *prop = (float*) calloc(buffer_size * mz, sizeof(float));  // input material
            ...
            for (i = 0; i < m.nxt; ++i) {
                for (j = 0; j < m.nyt; ++j) {
                    ...
                    if (mesh_out == 1) {           
                        ...
                        int idx_z = (int) ((f[local_pos] - mapping)/ h);
                        if (idx_z >= mz) {
                            fprintf(stderr, "Error! Curvilinear grids deeper than property "
                                    "mesh\nAborting...\n");
                            fflush(stderr);
                            MPI_Finalize();
                            return(-1);
                        ...
                        memcpy(buffer_m + nvars * (j * m.nxt + i), prop + pos, nvars * sizeof(float));
                    }
                }
            }
            if (mesh_out == 1) {
                MPICHK(MPI_File_write_all(fm, buffer_m, buffer_size,  MPI_FLOAT,
                                        &filestatus));
            }
#if WRITE_COORD
