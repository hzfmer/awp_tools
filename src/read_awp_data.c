...
void read_awp_timeseries(char *fname, int nx, int ny, int nz, int* xi, int* yi, int* zi, 
    ...
    MPI_Offset * disp;
    disp = (MPI_Offset *) malloc(nsites * sizeof(MPI_Offset));
    int i;
    for (i = 0; i < nsites; ++i) {
        disp[i] = (MPI_Offset) sizeof(float) * ((MPI_Offset) nx*ny*zi[i] + (MPI_Offset) nx*yi[i] + (MPI_Offset) xi[i];
    }
    ierr = MPI_Type_create_indexed_block(nsites, nt, disp, MPI_FLOAT, &filetype);  
    error_check(ierr, "MPI_Type_create_indexed_block()");
    ...
    ierr=MPI_File_set_view(fh, 0, MPI_FLOAT, filetype, "native", MPI_INFO_NULL);
