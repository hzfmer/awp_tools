...
MPI_Datatype data_type(const struct Mpi *mpi, int nx, int ny, int nz, int nt, int ysplit, int yoff);
...
int main(int argc, char *argv[])
{
    ...
    while ((c = getopt(argc, argv, "fgl:h:s:e:p:x:y:z:")) != -1)
    {
        switch (c)
        {
            ...
            case 'z':
                ...
                break;
            case '?':
                fprintf(stdout, "usage: %s [-f] [-l lp] [-h hp] [-s low] [-e high] [-x px] [-y py] [-z pz] [-p ysplit]\n", argv[0]);
                exit(0);
            default:
                abort();
        }
    ...
    if ((nx / px != m.nxt) || (ny / py != m.nyt) || ((ny % (py * ysplit)) != 0) || (nz / pz != m.nzt)) {
    ...
    if (m.rank == 0)
    {
        ...
        fprintf(stdout, "MPI Comm m: nxt=%d, nyt=%d, nzt=%d, ntt=%d, dim=(%d, %d, %d), period=(%d, %d, %d), reorder=%d\n", m.nxt, m.nyt, m.nzt, m.ntt, m.dim[0], m.dim[1], m.dim[2], m.period[0], m.period[1], m.period[2], m.reorder); 
        /* number of points to process per read operation */
        fprintf(stdout, "dofilt=%d, lp=%f, hp=%f, filter type=%s, low=%f, high=%f, px=%d, py=%d, pz=%d, ysplit=%d\n", dofilt, lp, hp, ftype, low, high, px, py, pz, ysplit);
        fflush(stdout);
    }
    ...
    csize = m.nxt * m.nyt * m.nzt / ysplit;
    if (m.rank == 0) {
    ...
    for (m = 1; m <= ysplit; ++l) {
        MPI_Datatype read_dtype = data_type(&m, nx, ny, nz, size[3], ysplit, (m - 1) * ny / ysplit);
        MPI_Datatype write_dtype = data_type(&m, nx, ny, nz, 1, ysplit, (m - 1) * ny / ysplit);
...
MPI_Datatype data_type(const struct Mpi *mpi, int nx, int ny, int nz, int nt, int ysplit, int yoff)
{
    ...
    int new[4] = {nt, mpi->nzt, mpi->nyt / ysplit, mpi->nxt};
    int offset[4];
    offset[0] = 0;
    offset[1] = new[1] * mpi->coord[2];
    offset[2] = new[2] * mpi->coord[1];
